...
const Calendar = ({ tasks, currentDate, onDateChange }) => {
  ...
  tasks.forEach(t => {
    ...
    if (ds && ds.startsWith(monthPrefix)) {
      ...
      <Card.Header>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexShrink: 0 }}>
            <Button variant="outline-primary" size="sm" onClick={prevMonth} style={{ padding: '4px 10px' }}>
              <i className="bi bi-chevron-left"></i>
            </Button>
            <span style={{
              color: 'var(--text-primary)', fontSize: '0.85rem', fontWeight: 600,
              minWidth: '130px', textAlign: 'center', display: 'inline-block'
            }}>
              {monthYear}
            </span>
            <Button variant="outline-primary" size="sm" onClick={nextMonth} style={{ padding: '4px 10px' }}>
      ...
      <Card.Body>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', marginBottom: '8px', textAlign: 'center' }}>
          {['Su', 'Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa'].map(day => (
            <div key={day} style={{ color: 'var(--text-muted)', fontSize: '0.72rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px', padding: '4px 0' }}>
              {day}
            </div>
          ))}
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '5px' }}>
          {days.map((day, i) => {
            if (!day) {
              return <div key={`empty-${i}`} style={{ height: '34px', background: 'transparent', border: 'none' }} />;
            }
            ...
            const dotColor = allDone ? 'var(--green)' : hasOverdue ? 'var(--red)' : 'var(--blue)';
            ...
            return (
              <div
                ...
                style={{
                  height: '34px', display: 'flex', flexDirection: 'column',
